...
const processOwnerState = vi.hoisted(() => {
    ...
    return {
        ...
        readProcessOwner: vi.fn<
            (pid: number) => Promise<{ pid: number; processStartedAtMs: number } | null>
        >(async (pid: number) =>
            ensureLive({
                pid,
...
describe('shared real-browser sessions', () => {
    ...
    it('respects the caller timeout while waiting for the spawned process owner', async () => {
        ...
        processOwnerState.readProcessOwner.mockImplementation(async () => null)
