...
export function defineBrowserCoreConformanceSuite(options: BrowserCoreConformanceOptions): void {
  describe(options.name, () => {
    test.sequential(
      "models session/page/frame topology and popup relationships",
      async () => {
        ...
        try {
          ...
          const popupFrames = await harness.engine.listFrames({
            pageRef: popupResult.data.pageRef,
          });
          ...
          expect(
            pages.find((page) => page.pageRef === popupResult.data.pageRef)?.openerPageRef,
          ).toBe(pageResult.data.pageRef);
    ...
    test.sequential(
      "preserves DocumentRef on same-document navigation and rotates it on reload and cross-document navigation",
      async () => {
        const harness = await options.createHarness();
        try {
          const sessionRef = await harness.engine.createSession();
    ...
    test.sequential(
      "returns HTML and DOM snapshots with ordered attributes",
      async () => {
        const harness = await options.createHarness();
        try {
          const sessionRef = await harness.engine.createSession();
          ...
          expect(dom.shadowDomMode === "flattened" || dom.shadowDomMode === "preserved").toBe(true);
