...
export async function capturePageDomSnapshot(
  ...
  return {
    ...
    shadowBoundariesByBackendNodeId: buildShadowBoundaryIndex(domTreeResult.root),
...
export function findHtmlBackendNodeId(
  ...
  for (let index = 0; index < nodeNames.length; index += 1) {
    if (parseStringTable(captured.strings, nodeNames[index]) === "HTML") {
...
export function buildDomSnapshot(
  ...
  const layoutByNodeIndex = new Map<
    number,
    { readonly rect?: Rect; readonly paintOrder?: number }
  >();
  ...
  for (let index = 0; index < nodeCount; index += 1) {
    ...
    const nodeRef =
      backendNodeId === undefined ? undefined : nodeRefResolver(document, backendNodeId);
    ...
    nodes.push({
      ...
      nodeValue: parseStringTable(captured.strings, captured.rawDocument.nodes.nodeValue?.[index]),
...
function buildShadowBoundaryIndex(root: DomTreeNode): ReadonlyMap<number, ShadowBoundaryInfo> {
  const boundaries = new Map<number, ShadowBoundaryInfo>();
  visitTree(root, undefined);
  return boundaries;

  function visitTree(node: DomTreeNode, shadowHostBackendNodeId: number | undefined): void {
    if (node.backendNodeId !== undefined && shadowHostBackendNodeId !== undefined) {
      const shadowRootType = normalizeShadowRootType(node.shadowRootType);
      boundaries.set(node.backendNodeId, {
        shadowHostBackendNodeId,
        ...(shadowRootType === undefined ? {} : { shadowRootType }),
      });
    }
    for (const child of node.children ?? []) {
      visitTree(child, undefined);
    }
    for (const shadowRoot of node.shadowRoots ?? []) {
      visitTree(shadowRoot, node.backendNodeId);
    }
    if (node.contentDocument) {
      visitTree(node.contentDocument, undefined);
    }
  }
}

function visitTree(node: DomTreeNode, callback: (node: DomTreeNode) => void): void {
  callback(node);
  for (const child of node.children ?? []) {
    visitTree(child, callback);
  }
  for (const shadowRoot of node.shadowRoots ?? []) {
    visitTree(shadowRoot, callback);
  }
  if (node.contentDocument) {
    visitTree(node.contentDocument, callback);
  }
}
...
function rareStringValue(
  ...
  return parseStringTable(strings, data.value[matchIndex]);
}
